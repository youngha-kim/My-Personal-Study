...
export function PostDetail({ post }) {
  ...
  return (
    <>
      ...
      {updateMutation.isError && <h3 style={{ color: "red" }}>{post.title}</h3>}
     {updateMutation.isLoading && <h3 style={{ color: "black" }}>{post.title}</h3>}
     {updateMutation.isSuccess && <h3 style={{ color: "blue" }}>{post.title}</h3>}
      <p>{post.body}</p>
      <h4>Comments</h4>
      {data.map((comment) => (
        <li key={comment.id}>
